...
{
	UDATA localVar = 0;
	UDATA currentstartingSP  = (UDATA)&localVar;
	UDATA usedStack = 0;
						   ...
						   U_8 * dataEnd, U_8 * segment, U_8 * segmentEnd, U_8 ** pIndex, U_8 ** pFreePointer, U_32 flags, UDATA * syntheticFound, UDATA availableOSStackSpace, UDATA startingSP)
{
	...
	for (i = 0; i < attributesCount; i++) {
		...
		case CFR_ATTRIBUTE_Code:
			...
			if ((result = readAttributes(classfile, &(code->attributes), code->attributesCount, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, NULL, availableOSStackSpace, sp)) != 0) {
				return result;
			}
			break;
		...
		case CFR_ATTRIBUTE_AnnotationDefault: {
			...
			result = readAnnotationElement(classfile, &((J9CfrAttributeAnnotationDefault *)attrib)->defaultValue,
					data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
			if (result != 0) {
				return result;
			}
		}
		...
		case CFR_ATTRIBUTE_RuntimeVisibleAnnotations: {
			...
			if (length > 1) {
				...
				result = readAnnotations(classfile, annotations->annotations, annotations->numberOfAnnotations, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
			}
		...
		case CFR_ATTRIBUTE_RuntimeVisibleParameterAnnotations: {
			...
			for (j = 0; j < annotations->numberOfParameters; j++, parameterAnnotations++) {
				...
				result = readAnnotations(classfile, parameterAnnotations->annotations,	parameterAnnotations->numberOfAnnotations, data, dataEnd,
						segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
			...
			if (length > 1) {
				...
				*/
				for (j = 0; j < annotations->numberOfAnnotations; j++, typeAnnotations++) {
					result = readTypeAnnotation(classfile, typeAnnotations, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
					if (BCT_ERR_NO_ERROR != result) {
						break;
					}
				}
		...
		case CFR_ATTRIBUTE_Record:
			...
			for (j = 0; j < record->numberOfRecordComponents; j++) {
				...
				result = readAttributes(classfile, &(recordComponent->attributes), recordComponent->attributesCount, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, NULL, availableOSStackSpace, sp);
				if (result != 0) {
					return result;
				}
			}
...
readMethods(J9CfrClassFile* classfile, U_8* data, U_8* dataEnd, U_8* segment, U_8* segmentEnd, U_8** pIndex, U_8** pFreePointer, U_32 flags, UDATA availableOSStackSpace, UDATA startingSP)
{
	...
	for (i = 0; i < classfile->methodsCount; i++) {
		...
		/* Read the attributes. */
		if ((result = readAttributes(classfile, &(method->attributes), method->attributesCount, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, &syntheticFound, availableOSStackSpace, sp))!= 0) {
			return result;
		}
		if (syntheticFound) {
...
readFields(J9CfrClassFile* classfile, U_8* data, U_8* dataEnd, U_8* segment, U_8* segmentEnd, U_8** pIndex, U_8** pFreePointer, U_32 flags, UDATA availableOSStackSpace, UDATA startingSP)
{
	...
	for (i = 0; i < classfile->fieldsCount; i++, field++) {
		...
		/* Read the attributes. */
		if ((result = readAttributes(classfile, &(field->attributes), field->attributesCount, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, &syntheticFound, availableOSStackSpace, sp)) != 0) {
			return result;
		}
		if (syntheticFound) {
	...
	U_8* data, UDATA dataLength, U_8* segment, UDATA segmentLength, U_32 flags, U_8** segmentFreePointer, void *verboseContext, UDATA findClassFlags, UDATA romClassSortingThreshold, UDATA availableOSStackSpace)
{
	...
	/* Read the pool. */
	if ((result = readPool(classfile, data, dataEnd, segment, segmentEnd, &index, &freePointer, availableOSStackSpace, sp)) != 0) {
	...
	/* Read the fields. */
	if((result = readFields(classfile, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp)) != 0) {
		Trc_BCU_j9bcutil_readClassFileBytes_Exit(result);
		return result;
	}
	...
	/* Read the methods. */
	if((result = readMethods(classfile, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp)) != 0) {
		Trc_BCU_j9bcutil_readClassFileBytes_Exit(result);
		return result;
	}
	...
	/* Read the attributes. */
	if((result = readAttributes(classfile, &(classfile->attributes), classfile->attributesCount, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, &syntheticFound, availableOSStackSpace, sp)) != 0) {
		Trc_BCU_j9bcutil_readClassFileBytes_Exit(result);
		return result;
	}
	if (syntheticFound) {
						   ...
						   U_8 * dataEnd, U_8 * segment, U_8 * segmentEnd, U_8 ** pIndex, U_8 ** pFreePointer, U_32 flags, UDATA availableOSStackSpace, UDATA startingSP)
{
	...
	result = readAnnotations(classfile, &(tAnn->annotation), 1, data,
			dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
	*pFreePointer = freePointer;
	*pIndex = index;
	return result;
						   ...
						   U_8 * dataEnd, U_8 * segment, U_8 * segmentEnd, U_8 ** pIndex, U_8 ** pFreePointer, U_32 flags, UDATA availableOSStackSpace, UDATA startingSP)
{
	...
	case '@':
		...
		result = readAnnotations(classfile, annotation, 1, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
		if (BCT_ERR_NO_ERROR != result) {
			return result;
		}
	...
	case '[':
		...
		for (j = 0; j < array->numberOfValues; j++, arrayWalk++) {
			result = readAnnotationElement(classfile, arrayWalk, data, dataEnd, segment, segmentEnd, &index, &freePointer, flags, availableOSStackSpace, sp);
			if (result != 0) {
				return result;
			}
		}
